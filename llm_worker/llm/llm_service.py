...
def merge_fn(summaries_group: list[str]) -> str:
    ...
    АЛГОРИТМ (выполняй шаг за шагом):
      ...
      Шаг 2. Для каждого id найди все его вхождения во всех частях и объедини их содержимое в один блок, сохранив логику:
               - массив + массив → объединить элементы, убрать точные дубли
               - строка + строка → объединить в один текст, убрать повторяющуюся информацию
...
def build_prompt(form: dict, transcript: str) -> str:
    ...
    АЛГОРИТМ (выполняй шаг за шагом):
      ...
      Шаг 2. Для каждого блока найди в транскрипции информацию, которая относится  к теме этого блока (тема задана в поле title). Если для блока в форме указана инструкция по заполнению, следуй ей при поиске информации в транскрипте.
      Шаг 3. Запиши найденную информацию ПРЯМО в поле "content", тип значения должен соответствовать виду блока — без дополнительных обёрток и ключей.
    ...
    ТРЕБОВАНИЯ К СОДЕРЖАНИЮ БЛОКОВ:
      - Сформулируй содержимое блока от третьего лица по тезисам и ключевым моментам. Не пересказывай речь участников дословно, если этого явно не указано.
